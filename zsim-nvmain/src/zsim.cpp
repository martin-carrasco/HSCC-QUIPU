...
//call back function called by fPtrs[tid].loadPtr
...
//ffiEnabled && ffiNFF => ffiEntryPtrs
//ffiEnabled && ffiEntryPtrs => ffiPtrs
//ffiDisabled => ffPtrs
static const InstrFuncPtrs& GetFFPtrs() {
...
VOID Instruction(INS ins) {
    ...
    if (!procTreeNode->isInFastForward() || !zinfo->ffReinstrument) {
        AFUNPTR LoadFuncPtr = (AFUNPTR) IndirectLoadSingle;	//nop or joinAndLoad
        AFUNPTR StoreFuncPtr = (AFUNPTR) IndirectStoreSingle;	//nop or joinAndStore

        AFUNPTR PredLoadFuncPtr = (AFUNPTR) IndirectPredLoadSingle;	//nop or join and predLoad
        AFUNPTR PredStoreFuncPtr = (AFUNPTR) IndirectPredStoreSingle; //nop or join and preStore

        if (INS_IsMemoryRead(ins)) {
			//tread instructions as predicated condition move floating conditional
			// move rep string ops
            if (!INS_IsPredicated(ins)) {
				//insert a call to LoadFuncPtr relative to instruction ins
	...
	//instruction is rdtsc or rdtscp
    if (INS_IsRDTSC(ins)) {
...
//when call Trace, call BblFunc of ooo_core.cpp or simple_core.cpp, etc.
VOID Trace(TRACE trace, VOID *v) {
	//no fast forwarding
	//no reintrument of fast fowarding
    if (!procTreeNode->isInFastForward() || !zinfo->ffReinstrument) {
        ...
        for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl)) {
			...
			//						   for offering optimized call linkages
			//IARG_THREAD_ID: analysis routine argument;
...
void SimThreadStart(THREADID tid) {
	...
	info("Simulate Thread %ld TID", syscall(SYS_gettid));
...
/*
 * @function: delete global allocated object
 */
void DeleGlobalObjects()
{
	if( zinfo->memory_node )
		delete zinfo->memory_node;
	for( unsigned i=0 ; i< zinfo->memoryControllers.size();i++)
	{
		if( zinfo->memoryControllers[i])
			delete zinfo->memoryControllers[i];
	}
	if( zinfo->pinCmd)
		delete zinfo->pinCmd;
	if( zinfo->rootStat)
		delete zinfo->rootStat;
	if( zinfo->periodicStatsBackend)
		delete zinfo->periodicStatsBackend;
	if( zinfo->statsBackend)
		delete zinfo->statsBackend;
	if( zinfo->eventualStatsBackend)
		delete zinfo->eventualStatsBackend;
	if( zinfo->compactStatsBackend)
		delete zinfo->compactStatsBackend;
	if( zinfo->processStats)
		delete zinfo->processStats;
	if( zinfo->profSimTime)
		delete zinfo->profSimTime;
	if( zinfo->profHeartbeats)
	 	delete zinfo->profHeartbeats;
	if( zinfo->procTree)
		delete zinfo->procTree;
	if( zinfo->buddy_allocator)
		delete zinfo->buddy_allocator;
	if( zinfo->page_table_walker)
		delete zinfo->page_table_walker;
	if( zinfo->procExited)
		gm_free((void*)zinfo->procExited);
	if( zinfo->procArray)
	{
		for( unsigned i= 0; i<procIdx; i++)
		{
			if( zinfo->procArray[i])
				delete zinfo->procArray[i];
		}
		gm_free((void*)zinfo->procArray);
	}
	if( zinfo->cores)
	{
		...
		{
			if( zinfo->cores[i])
				delete zinfo->cores[i];
		}
		gm_free((void*)zinfo->cores);
	}
	for( unsigned i=0 ; i<MAX_PORT_DOMAINS;i++)
	{
		if( zinfo->portVirt[i])
			delete zinfo->portVirt[i];
	}
	if( zinfo->outputDir)
		gm_free((void*)zinfo->outputDir);
}
/** Finalization **/
    ...
    if (procIdx == 0) {
        ...
        if (zinfo->hasNVMain) {
            for(uint32_t i = 0; i < zinfo->numMemoryControllers; i++) {
				std::cout<<"print nvmain stat"<<std::endl;
		...
		if( zinfo->cores )
		{
			for(unsigned i=0 ; i<zinfo->numCores;i++)
			{
				...
				i_core->getInsCache()->calculate_stats();
				i_core->getDataCache()->calculate_stats();
				if( i_core->getInsTlb() )
				{
					total_access_time += i_core->getInsTlb()->calculate_stats();
				}
		...
		info("total TLB access time is: %lu",total_access_time);
		if( zinfo->pg_walkers)
		{
			for( unsigned i=0; i<zinfo->numCores; i++)
			{
...
int main(int argc, char *argv[]) {
	...
	//set the parent process death signal of the current process to arg2
    if (prctl(PR_SET_PDEATHSIG, 9 /*SIGKILL*/) != 0) {
        panic("prctl() failed");
    }
	//out put string to zsim log file
    ...
    if (!masterProcess && zinfo->attachDebugger) {
        notifyHarnessForDebugger(zinfo->harnessPid);	//debuger pid
    }
	
	info("process id is %d",procIdx);
	info("number of processes is %d",zinfo->numProcs);
    assert((uint32_t)procIdx < zinfo->numProcs);
    procTreeNode = zinfo->procArray[procIdx];	//get process node from process tree
	...
	//when a thread start , call ThreadStart function
    PIN_AddThreadStartFunction(ThreadStart, 0);
	//when a thread finished , call ThreadFini function
    PIN_AddThreadFiniFunction(ThreadFini, 0);
	//called immediatedly before execution of a system call
    PIN_AddSyscallEntryFunction(SyscallEnter, 0);
	//called immediately after execution of a system call
    PIN_AddSyscallExitFunction(SyscallExit, 0);
	//register a notification function that is called immediately before the application changes context due to receipt of a a synchronous event such as unix signal or windows APC
    PIN_AddContextChangeFunction(ContextChange, 0);
	//call func immediately before the application exits
    PIN_AddFiniFunction(Fini, 0);

    //Follow exec and fork, is called before a child/exec-ed process is starting to execute
    PIN_AddFollowChildProcessFunction(FollowChild, 0);
	//is called when a application forks a new process
