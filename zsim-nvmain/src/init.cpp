...
BaseCache* BuildCacheBank(Config& config, const string& prefix, g_string& name, uint32_t bankSize, bool isTerminal, uint32_t domain) {
    ...
    } else if (replType == "WayPart" || replType == "Vantage" || replType == "IdealLRUPart") {	//partition cache
    ...
    if (!isTerminal) {
        ...
        //isTerminal=true, Filter cache optimization(has no children,then is terminal is true , l1i cache and l1d cache has no children)
...
static void InitSystem(Config& config) {
	...
	//travel all caches and init cachegroup for them, start from LLC
    while (!fringe.empty()) {
        string group = fringe.front();
        fringe.pop_front();
		//travel cache whose name is group
		//terminal cache has no child
        bool isTerminal = (childMap.count(group) == 0); //if no children, connected to cores
		//debug_printf("group %s terminal : %d", group.c_str(), isTerminal);
        if (cMap.count(group))
		{
			debug_printf("The cache 'tree' has a loop at %s", group.c_str());
			panic("The cache 'tree' has a loop at %s", group.c_str());
		}
		//build cache group for cache whose name is group 
	...
	//init memory_size of zinfo to 0 before build memory controller  
	zinfo->memory_size = 0;
    ...
    uint32_t childId = 0;
	//there only has a LLC cache
	//cMap[llc]-> pointer pointed to LLC cachegroup
	//(*cMap[llc])[0]: cache 0 of LLC cache group
	//attention: there will only has a cache in LLC cache group
    for (BaseCache* llcBank : (*cMap[llc])[0]) {
        llcBank->setParents(childId++, mems, network);
    }
    // Rest of caches
    for (const char* grp : cacheGroupNames) {
		//l1i cache or l1dcache, do nothing
        ...
        for (string child : childMap[grp]) 
			childCaches.insert(childCaches.end(), cMap[child]->begin(), cMap[child]->end());

        uint32_t children = childCaches.size();
        assert(children);
		//each childer/parents children cache connect to a parent cache
		//eg.: l2 cache number is 2,l3 cache number is 1,then childernPerParent is 2, that is to say, 2 l2 cache connect to 1 l3 cache
		...
		//travel child of childMap[grp],
		//if some child has no children(l1i cache , l1d cache),
		//or some child is Prefetcher, then set terminalChildren true
        for (string child : childMap[grp]) 
			terminalChildren &= (childMap.count(child) == 0 || config.get<bool>("sys.caches." + child + ".isPrefetcher", false));
        if (terminalChildren) {
			...
			//l1i cache , l1d cache adjust
			//l1i_t cache,l1d_t cache-> l2 cache
			//can't l1i_1,l1i_2 cache connect to l2_1
			//l1d_1,l1d_2 cache connect to l2_2
            for (uint32_t i = 0; i < children; i++) 
				childCaches[i] = tmp[(i % childrenPerParent)*stride + i/childrenPerParent];
        }

        for (uint32_t p = 0; p < parents; p++) {
            g_vector<MemObject*> parentsVec;
			//push back pointer of all cache bank of parent caches to parentsVec
            ...
            for (uint32_t c = p*childrenPerParent; c < (p+1)*childrenPerParent; c++) {
				//connect banks between each pair of parent cache and child cache 
                for (BaseCache* bank : childCaches[c]) {
					//childId is related to cache bank
    ...
    for (const char* grp : cacheGroupNames) {
		//l1i cache and l1d cache has no child
	...
	//map between terminal_cache_name and number of l1 icaches or l1 dcaches has already been assigned to cores
	...
	//map between core name and core group 
    ...
    for (const char* group : coreGroupNames) {
        ...
        if (type != "Null") {
			...
			if( tlb_type == "HotMonitorTlb")
			{
				...
				zinfo->life_time = config.get<unsigned>("sys.tlbs.life_time",5);
				hot_monitor_tlb = gm_memalign<HotMonitorTlb<ExtendTlbEntry> >(CACHE_LINE_BYTES,2*cores);
			}
			int tlb_id = 0;
            ...
            for (uint32_t j = 0; j < cores; j++) {
				...
				//allocate the one instruction to a core
				...
				BaseTlb* itlb = NULL;
				BaseTlb* dtlb = NULL;
				string pgt_prefix="sys.pgt_walker.";
				string pgt_name="sys.pgt_walker";
				if( config.exists( "sys.tlbs") )
				{
					if( config.exists(pgt_name))
					{
						//default paging mode is Legacy-Normal
						string mode_str = config.get<const char*>(pgt_prefix+"mode" , "Legacy-Normal");
						PagingStyle mode = string_to_pagingmode(mode_str.c_str());
						//zinfo->paging_mode = mode;
						stringstream ss;
						ss << pgt_name << coreIdx;
					...
					for( const char* grp : tlb_group_names)
					{
						...
						if( tlb_type == "CommonTlb")
						{
							tlb = new (&common_tlb[tlb_id]) CommonTlb<TlbEntry>( tlb_name.c_str() , tlb_size , tlb_hit_lat , tlb_res_lat, stringToPolicy(evict_policy_str));
						}
						if( tlb_type == "HotMonitorTlb")
						{
                ...
                if (type == "Simple") {
                    ...
                    assert(type == "OOO");
					//placement new
                    OOOCore* ocore = new (&oooCores[j]) OOOCore(ic, dc, itlb , dtlb , name , j);	//icache , dcache , name,id
					ocore->setCoreId(j);
	...
	//init state for all banks of all caches
	...
	if( config.exists("sys.tlbs"))
	{
		...
		if( config.exists(c_zone_sec))
		{
			...
			if ( config.exists(c_zone_dma32))
				zinfo->max_zone_pfns[Zone_DMA32] = ((2<<20)*config.get<Address>(c_zone_dma32))>>(zinfo->page_shift);			else 
	...
	if( zinfo->counter_tlb == true)
	{
		...
		//simple configure, no large page, 4KB pcm related to 4KB dram
		zinfo->block_size = zinfo->page_size;
		zinfo->block_shift = zinfo->page_shift;
		...
		zinfo->adjust_interval = (uint64_t)config.get<unsigned>("sys.adjust_interval",1)*10000;
		/********************************/
		if( zinfo->proc_fairness )
			zinfo->dram_evict_policy = Fairness;
		else
			zinfo->dram_evict_policy = Equal;
		//###config HSCH-DYN
		zinfo->dynamic_threshold = config.get<bool>("sys.dynamic_threshold",true);
		std::cout<<"adjust interval is:"<<std::dec<<zinfo->adjust_interval<<std::endl;
		//###config HSCH-MQ
		zinfo->multi_queue = config.get<bool>("sys.multi_queue", false);
		/**********************************/
	}
	//default:enable share memory
	/*zinfo->enable_shared_memory = config.get<bool>("sys.enable_shared_memory", true);
...
static void PreInitStats() {
    ...
    zinfo->rootStat->init("root", "Stats");	//name="root",desc="Stats"
}
...
void SimInit(const char* configFile, const char* outputDir, uint32_t shmid) {
	...
	//MAX_THREADS is defined as 2048
    ...
    debug_printf("create scheduler stats");
    ...
    debug_printf("Initialization global stats");
    InitGlobalStats();
    debug_printf("Initialization global stats done");
	...
	debug_printf("new process stats");
