...
bool Decoder::canFuse(INS ins) {
	//get opcode
    xed_iclass_enum_t opcode = (xed_iclass_enum_t) INS_Opcode(ins);
	//opcode!==XED_IClass_##CMP && opcode!=XED_CATEGORY_##TEST
...
BblInfo* Decoder::decodeBbl(BBL bbl, bool oooDecoding) {
    uint32_t instrs = BBL_NumIns(bbl);	//number of instructions within a BBL
    uint32_t bytes = BBL_Size(bbl);	//size of bbl code , in bytes
    BblInfo* bblInfo;
    ...
    if (oooDecoding) {
		...
		//64b mode: XED_MACHINE_MODE_LONG_64
		//			XED_ADDRESS_WIDTH_64b
		//argv 4: the stack address width of type xed_address_width_enum_t
        xed_state_init(&dstate, XED_MACHINE_MODE_LONG_64, XED_ADDRESS_WIDTH_64b, XED_ADDRESS_WIDTH_64b);
		//zero the decode structure , but set the maching state/mode information
        ...
        for (INS ins = BBL_InsHead(bbl); INS_Valid(ins); ins = INS_Next(ins)) {
            ...
            if (Decoder::canFuse(ins)) {
                ...
                instrAddr.push_back(INS_Address(ins));	//address
                instrBytes.push_back(INS_Size(ins));	//instruction size
                instrUops.push_back(uopVec.size() - prevUops);	//uops num
                instrDesc.push_back(ins);	//instruction
