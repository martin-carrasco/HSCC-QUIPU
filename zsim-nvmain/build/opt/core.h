...
class Core : public GlobAlloc {
	...
	public:
		...
		unsigned coreId;
    public:
        explicit Core(g_string& _name) : lastUpdateCycles(0), lastUpdateInstrs(0), name(_name),itlb(NULL),dtlb(NULL) {}
		virtual void setCoreId( unsigned core_id)
		{
			coreId = core_id;
		}

		virtual void SetProcIdx( uint32_t proc )
		{
			procIdx = proc;
		}
		virtual uint32_t GetProcIdx( )
		{
			return procIdx;
		}
